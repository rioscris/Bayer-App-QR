...
export const Popup = ({visible, title, text, width, type, onPress}) => {
    return (
        <Overlay isVisible={visible} animationType={"fade"} overlayStyle={{width: width}}>
            <View style={{flexDirection: "column", justifyContent: "space-evenly", padding: 10}}>
                <Icon name={type === 'approved' ? 'check-circle' : type === 'rejected' ? 'x-circle' : 'alert-circle'}
                    ...
                    size={80}
                    />
                <View style={{alignContent: 'center', flexDirection: "column", justifyContent: "center", padding: 20}}>
                    {title && 
